...
async function getJson(url){
  const r = await fetch(url, {headers: {accept: "application/json", "user-agent": "tidewatch/1.0 (+https://www.usetidewatch.org)"},
...
// Robinhood trades stock tokens 24 hours a day, five days a week: Sunday 8 pm to Friday 8 pm New York time.
// Outside that window the share price is frozen, so gaps say nothing about mispricing.
...
const THIN = 10000;          // pools shallower than this move on small trades
const STALE_MS = 30 * 60000; // a reference price older than this is treated as paused

// A gap worth acting on: market open, fresh share price, not halted, a pool deep enough to matter.
const usable = (s, now = Date.now()) => s.gap != null && !s.halted && s.liquidity >= THIN &&
  s.refAt && now - Date.parse(s.refAt) < STALE_MS;

// For the bot and the scheduled check: our CDN-cached endpoint first, the sources directly as backup.
async function currentStocks(site){
  try {
    const j = await getJson(site.replace(/\/$/, "") + "/api/stocks");
    if (j && j.data && j.data.length) return j.data;
  } catch (e) {}
  return stockBoard();
}

module.exports = {stockBoard, currentStocks, marketOpen, usable, THIN};
