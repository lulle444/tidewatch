...
// The public endpoint answers 429 when calls come in bursts, so calls are spaced out a little.
const GAP = process.env.RPC_URL ? 0 : 120;
let lastCall = 0;
async function post(body, tries = 0){
  const wait = lastCall + GAP - Date.now();
  lastCall = Math.max(Date.now(), lastCall + GAP);
  ...
  if (limited || (!j && r.status >= 500)) {
    // a heavy log query keeps answering 429, so give up sooner and let logs() split the range
    if (tries >= (limited ? 5 : 7)) throw new Error(`rpc ${limited ? 429 : r.status}: ${text.slice(0, 200)}`);
    await sleep(Math.min(20000, 500 * 2 ** tries));
...
async function logs(filter, from, to, step = 4000){
  ...
  for (let a = from; a <= to; a += step) {
    ...
    try {
      ...
      if (step <= 250 || !/range|limit|too many|exceed|size|timed? ?out|rpc 429/i.test(e.message)) throw e;
