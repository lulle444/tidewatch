...
const SITE = "https://www.usetidewatch.org";
const T = {
...
// refOf maps a symbol to the share's price: a trade priced at under half or over twice it is dropped, because that
// is a pool quoting in a fake "USDC" or a token with odd decimals, not a real trade.
async function readTrades(from, to, pools, symOf, eth, refOf = {}){
  ...
  for (const [hash, t] of txs) {
    for (const [tok, e] of t) {
      ...
      if (!sym) continue;
      const ref = refOf[sym];
      if (ref > 0 && !(e.usd / e.qty > ref / 2 && e.usd / e.qty < ref * 2)) continue;
...
async function indexChunk(from, to, pools, symOf, eth, refOf){
  const out = {};
  for (const t of await readTrades(from, to, pools, symOf, eth, refOf)) {
...
async function liveTrades(){
  ...
  const symOf = {}, refOf = {};
  for (const s of stocks) { symOf[s.address.toLowerCase()] = s.symbol; if (s.ref > 0 || s.onchain > 0) refOf[s.symbol] = s.ref || s.onchain; }
  const bots = new Set();
  ...
  const trades = (await readTrades(latest - LIVE_BLOCKS, latest, pools, symOf, eth, refOf))
...
async function run({budgetMs = 200e3} = {}){
  ...
  try {
    ...
    if (peek && peek.full && peek.priced && !(peek.redo || []).length && peek.hi >= lastDone) return {skipped: "up to date", state: peek, nextChunkInBlocks: (lastDone + 2) * CHUNK - 1 - latest};
    const stocks = await currentStocks(SITE);
    const symOf = {}, prices = {}, refOf = {};
    for (const s of stocks) {
      symOf[s.address.toLowerCase()] = s.symbol;
      const p = s.onchain || s.ref;
      if (p > 0) prices[s.symbol] = p;
      if (s.ref > 0 || p > 0) refOf[s.symbol] = s.ref || p;
    }
    ...
    const now = Date.now();
    // Chunks indexed before trades were checked against the share price can hold mispriced trades (one pool added
    // $4B to the week in nine hours). Find chunks with far more volume than usual, take them out of the windows
    // now, and index them again below.
    if (state && !state.priced) {
      const idxs = Object.keys(aggs["7d"].chunks);
      const buckets = {}, vols = {};
      for (const idx of idxs) {
        buckets[idx] = ungz(await redis("GET", K.chunk(idx))) || {};
        vols[idx] = Object.values(buckets[idx]).reduce((t, syms) => t + Object.values(syms).reduce((u, v) => u + v[2], 0), 0);
      }
      const sorted = idxs.map(i => vols[i]).sort((a, b) => a - b), median = sorted[sorted.length >> 1] || 0;
      const bad = idxs.filter(i => vols[i] > Math.max(5 * median, 50e6));
      for (const idx of bad) for (const w of Object.keys(WINDOWS)) {
        if (!aggs[w].chunks[idx]) continue;
        merge(aggs[w].w, buckets[idx], -1);
        delete aggs[w].chunks[idx];
      }
      state = {...state, priced: true, redo: bad.map(Number)};
      purged = bad;
    }
    const todo = [];
    ...
    const add = async (idx) => {
      ...
      const bucket = await indexChunk(from, to, pools, symOf, eth, refOf);
    ...
    try {
      ...
      // chunks taken out above, indexed again with the price check. One that keeps failing goes to the back of the
      // queue and is given up after three tries, so it can't hold up the rest of the history.
      const tried = new Set(), paused = state && state.pauseUntil > Date.now();
      while (!paused && state && state.redo && state.redo.length && Date.now() - started < budgetMs) {
        const idx = state.redo[0];
        if (tried.has(idx)) break;
        tried.add(idx);
        try {
          await add(idx);
          state = {...state, redo: state.redo.slice(1)};
        } catch (e) {
          const fails = {...(state.redoFails || {})};
          fails[idx] = (fails[idx] || 0) + 1;
          state = {...state, redoFails: fails, redo: fails[idx] >= 3 ? state.redo.slice(1) : state.redo.slice(1).concat(idx)};
          failed = `redo ${idx}: ` + String(e.message || e).slice(0, 300);
        }
      }
      // fill in older history while there is time left, back to the longest window
      const oldest = now - Math.max(...Object.values(WINDOWS));
      while (!paused && state && !state.full && Date.now() - started < budgetMs && state.lo > 0 && state.hi >= lastDone) {
        if (await add(state.lo - 1) < oldest) state.full = true;
      }
    } catch (e) {
      failed = String(e.message || e).slice(0, 400);
    }
    // The public RPC answers old-history queries with 429 for long stretches. When the newest hours are in, rest the
    // history work for two hours instead of spending every run on it (the 7-day window also fills in as time passes).
    if (failed && /429/.test(failed) && state && state.hi >= lastDone) {
      state = {...state, pauseUntil: Date.now() + 2 * HOUR};
      failed = null;
    }
    ...
    return {latest, indexed: done, purged, state, error: failed, newPools: found, stockPools: Object.keys(pools).length, eth, ms: Date.now() - started,
