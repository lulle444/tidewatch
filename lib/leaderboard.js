...
async function indexChunk(from, to, pools, symOf, eth){
  ...
  for (const l of s3.concat(s4)) {
    ...
    const e = t.get(m.s) || {qty: 0, usd: 0, n: 0};
  ...
  for (const [hash, t] of txs) {
    for (const [tok, e] of t) {
      ...
      const sym = symOf[tok];
      if (!sym) continue;
      const side = got > 0 ? 1 : -1;
      const w = out[who] || (out[who] = {});
      const row = w[sym] || (w[sym] = [0, 0, 0, 0]);
      row[0] = r6(row[0] + side * e.qty);
      row[1] = r2(row[1] - side * e.usd);
      row[2] = r2(row[2] + e.usd);
      row[3] += 1;
    }
  }
  return out;
...
async function run({budgetMs = 200e3} = {}){
  ...
  try {
    ...
    return {latest, indexed: done, state, newPools: found, stockPools: Object.keys(pools).length, eth, ms: Date.now() - started,
...
module.exports = {run, indexChunk, merge, rank, discover, decodeString, K, CHUNK, WINDOWS};
